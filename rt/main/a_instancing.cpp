...
void addTree(Group* g, int trunkTessel, float trunkHeight, float trunkRadius,
  float crownHeight, float crownRadius, int crownTessel, int crownSteps)
{
  ...
  for(int i = 0; i < trunkTessel; ++i)
  {
    ...
    g->add(new Quad(
      start, end - start, Vector(0.0f, trunkHeight, 0.0f), nullptr, nullptr));
  }
    ...
    if(stepRadius > 0.0f)
    {
      ...
      for(int j = 0; j < crownTessel; ++j)
      {
        ...
        g->add(new Sphere(
          ...
          sphereRadius, nullptr, nullptr));
      }
    }
    g->add(new Sphere(
      Point(0.0f, trunkHeight, 0.0f), trunkRadius, nullptr, nullptr));
    g->add(
      new Sphere(Point(0.0f, trunkHeight + crownHeight - crownStep * 1.5, 0.0f),
        trunkRadius, nullptr, nullptr));
  }
}
...
void a_instancing()
{
  ...
  for(int i = 0; i < 11; ++i)
  {
    ...
    itree->translate(
      Vector(sin(angle) * circleRadius, 0.0f, cos(angle) * circleRadius));
    scene->add(itree);
  }
  ...
  scene->add(
    new InfinitePlane(Point(0, 0, 0), Vector(0, 1, 0), nullptr, nullptr));
  scene->rebuildIndex();

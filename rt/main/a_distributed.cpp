...
void renderCornellboxMotion(float scale, const char* filename, Camera* cam,
  Material* sphereMaterial, Material* floorMaterial, int numSamples = 1);
...
void a_distributed() {
    ...
    renderCornellbox(
      0.001f, "a7-1.png", cam, sphereMaterial1, floorMaterial1, 30);
    renderCornellbox(
      0.001f, "a7-2.png", cam, sphereMaterial2, floorMaterial2, 30);
    ...
    renderCornellboxMotion(
      0.001f, "motionblur.png", cam, sphereMaterial2, floorMaterial2);
}


void renderCornellboxMotion(float scale, const char* filename, Camera* cam,
  Material* sphereMaterial, Material* floorMaterial, int numSamples)
{
  Image                         img(400, 400);
  World                         world;
  RecursiveRayTracingIntegrator integrator(&world);
  ...
  scene->add(new Quad(Point(000.f, 000.f, 000.f) * scale,
    Vector(000.f, 000.f, 560.f) * scale, Vector(550.f, 000.f, 000.f) * scale,
  ...
  scene->add(new Quad(Point(000.f, 000.f, 560.f) * scale,
    Vector(000.f, 550.f, 000.f) * scale, Vector(550.f, 000.f, 000.f) * scale,
    nullptr, grey));  //back wall
  scene->add(new Quad(Point(000.f, 000.f, 000.f) * scale,
    Vector(000.f, 550.f, 000.f) * scale, Vector(000.f, 000.f, 560.f) * scale,
    nullptr, rightWallMaterial));  //right wall
  scene->add(new Quad(Point(550.f, 550.f, 000.f) * scale,
    Vector(000.f, -550.f, 000.f) * scale, Vector(000.f, 000.f, 560.f) * scale,
    nullptr, leftWallMaterial));  //left wall

  scene->add(new Sphere(Point(150.0f, 100.0f, 240.0f) * scale, 99.0f * scale,
    nullptr, sphereMaterial));
  scene->add(new Sphere(
    Point(450.0f, 50.0f, 50.0f) * scale, 49.0f * scale, nullptr, yellowmat));

  //tall box
  SimpleGroup* boxScene = new SimpleGroup();
  makeBox(boxScene, Point(265.f, 000.1f, 296.f) * scale,
    Vector(049.f, 000.f, 160.f) * scale, Vector(158.f, 000.f, -049.f) * scale,
    Vector(000.f, 330.f, 000.f) * scale, nullptr, grey);

  Motion* mot = new Motion(boxScene, engine);
  mot->setMoveRate(Vector(200.f, 0.f, 0.f) * scale, Vector( 50.f, 0.f, 0.f) * scale);
  scene->add(mot);
  ...
  Quad*     light = new Quad(Point(213 * scale, 549.99f * scale, 227 * scale),
    Vector(130 * scale, 0, 0), Vector(0, 0, 105 * scale), nullptr, lightsource);
  AreaLight als(light);
  world.light.push_back(&als);
  scene->add(light);
  ...
  engine.setFramesPerSecondAndTotalTime(1.f / 20.f, 2.5f);
  engine.render(img);
  img.writePNG(filename);
}

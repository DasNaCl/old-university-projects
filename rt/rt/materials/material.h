...
public:
  ...
  virtual RGBColor getReflectance(const Point& texPoint, const Vector& normal,
    const Vector& outDir, const Vector& inDir) const = 0;
  virtual RGBColor getEmission(const Point& texPoint, const Vector& normal,
    const Vector& outDir) const                      = 0;
  ...
  struct SampleReflectance
  {
    ...
    SampleReflectance(const Vector& direction, const RGBColor& weight)
      : direction(direction)
      , reflectance(weight)
    {
    }
  ...
  virtual SampleReflectance getSampleReflectance(const Point& texPoint,
    const Vector& normal, const Vector& outDir) const = 0;

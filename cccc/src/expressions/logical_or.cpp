...
logical_or::logical_or(token tok, expression::ptr left, expression::ptr right)
    : binary_printer(tok, std::move(left), std::move(right))
{  }

type::ptr logical_or::_infer(semantics::scope& s)
{
...
void logical_or::make_cf(irconstructer& irc, IRScope& irscope,
                         llvm::BasicBlock* trueBB,
                         llvm::BasicBlock* falseBB)
{
    llvm::BasicBlock* extraBB =
        llvm::BasicBlock::Create(irc.ctx, "extra-bb-logical-or",
                                 irc.current_fun, nullptr);
    lhs->make_cf(irc, irscope, trueBB, extraBB);
    irc._builder.SetInsertPoint(extraBB);
    rhs->make_cf(irc, irscope, trueBB, falseBB);
}

llvm::Value* logical_or::make_rvalue(irconstructer& irc, IRScope& irscope)
{
    llvm::BasicBlock* trueBB =
    ...
    lhs->make_cf(irc, irscope, falseBB, trueBB);

...
{
public:
  DummyIntegrator(World* world, bool global = false)
    : Integrator(world)
    , _global(global)
  {
  }
...
void a_local()
{
  ...
  PerspectiveCamera cam(Point(278.f * scale, 273.f * scale, -800.f * scale),
  ...
  scene.add(new Quad(Point(000.f, 000.f, 000.f) * scale,
    ...
    nullptr, nullptr));  //right wall
  scene.add(new Quad(Point(550.f, 550.f, 000.f) * scale,
    Vector(000.f, 000.f, 560.f) * scale, Vector(000.f, -550.f, 000.f) * scale,
    nullptr, nullptr));  //left wall

  scene.add(new Sphere(
    Point(150.0f, 100.0f, 150.0f) * scale, 99.0f * scale, nullptr, nullptr));

  //tall box
  makeBox(scene, Point(265.f, 000.1f, 296.f) * scale,
    Vector(158.f, 000.f, -049.f) * scale, Vector(049.f, 000.f, 160.f) * scale,
    Vector(000.f, 330.f, 000.f) * scale, nullptr, nullptr);
